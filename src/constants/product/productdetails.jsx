...
const ProductDetails = () => {
  ...
  //         {categories?.map((singleSlug, i) => {
  //           path = path + `/${singleSlug?.slug}`
  //           return (
  //             <Link
  //               key={i}
  //               href={path}
              ...
              images?.slice(0, sliceLength)?.map((singleImage, i) => {
                return (
                  <div className='rounded-[4px] overflow-hidden bg-black relative' key={i}>
                    <Image src={singleImage?.src} width={649} height={649} className='w-full h-full object-cover' alt={singleImage?.alt} />
                    {
                      singleImage?.video &&
