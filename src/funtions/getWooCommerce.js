...
export const getChildCategories = async (parentId) => {
    const url = `https://staging.afs-foiling.com/wp-json/wc/v3/products/categories?parent=${parentId}&per_page=100&lang=fr`;
    const response = await fetch(url, {
        headers: {
...
export const getProductsByCategoryId = async (ids, max, min) => {
    try {
        ...
        // If only one category → return all products immediately
        if (categories.length === 1) {
            return allProducts;
        }

        // AND condition for multiple categories
        const filteredProducts = allProducts.filter((product) => {
            const productCats = product.categories.map((c) => c.id);
            return categories.every((catId) => productCats.includes(catId));
        });
