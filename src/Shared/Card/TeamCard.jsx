...
const TeamCard = ({ member }) => {
    ...
    return (
        <div onMouseEnter={() => setHoverImg(secondPhotoUrl)} onMouseLeave={() => setHoverImg(photoUrl)} className='h-[492px] w-full relative rounded-sm overflow-hidden group text-white'>
            <Image src={hoverImg} height={492} width={351} alt={name} className='w-full h-full object-cover object-center scale-100 group-hover:scale-105 transition-all duration-500 z-10' />
            <div className='absolute left-0 right-0 bottom-0 pb-8 hidden group-hover:block backdrop-blur-sm z-20'>
